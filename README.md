...
8. 뉴스 데이터 대시보드 접속: [http://localhost:3001](http://localhost:3001)
    ...
    - **시간대 동기화 (Timezone Sync)**: 브라우저의 시간대를 자동으로 감지하여 모든 데이터(수집일, 발행일)를 사용자 로컬 시간 기준으로 표시하고 필터링합니다. 특히 '오늘' 데이터가 누락되지 않도록 로컬 날짜 기준으로 필터가 초기화됩니다.
    - **섹션 이름 표시**: 숫자 ID 대신 '정치', '경제' 등 직관적인 섹션 이름으로 데이터를 필터링하고 시각화합니다.
    - **시계열 전환**: 데이터 수집 시간(`Created At`) 또는 기사 발행 시간(`Article Date`) 기준으로 차트를 전환하여 볼 수 있습니다.
    - **처리 진행 상태 (Processing Progress)**: 원문 수집, 요약 생성, API 업로드의 각 단계별 진행 상황을 프로그레스 바를 통해 실시간으로 모니터링할 수 있습니다.
    - **실패 항목 초기화**: 'Reset' 버튼을 통해 실패 상태(`FAILED`)이거나 실패 사유(`fail_reason`)가 있는 모든 항목을 `PENDING`으로 일괄 초기화할 수 있습니다.
...
- **동작**: 요약이 완료된 기사를 외부 **Project RAG** 검색 API로 자동으로 업로드합니다.
- **주요 특징**:
    - **주기적 실행**: 5분마다 실행되어 요약은 완료되었으나 아직 업로드되지 않은 기사를 처리합니다.
