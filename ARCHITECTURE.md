...
- **Dashboard Frontend**: React + Vite 기반의 웹 애플리케이션입니다. Chart.js를 사용하여 수집 트렌드, 언론사별/섹션별 통계를 시각화하며, **브라우저 시간대 자동 동기화**, **최근 30일 기본 필터링**, **처리 단계별 프로그레스 바**, **시계열 기준(수집일 vs 발행일) 전환** 기능과 실패한 기사를 일괄 재시도할 수 있는 리셋 버튼을 제공합니다. (Port 3000)
...
8. **검색 API 업로드 (비동기)**:
    ...
    - **시간대 보정**: 검색 API 전송 시 `article_date`에 한국 표준시(KST) 오프셋(`+09:00`)을 명시하여 외부 시스템에서의 시간 오해를 방지합니다.
8. **데이터 시각화 및 모니터링**:
